...
usage()
{
  //fprintf("Usage: susan <in.pgm> <out.pgm> [options]\n\n");

  //fprintf("-s : Smoothing mode (default)\n");
  //fprintf("-e : Edges mode\n");
  //fprintf("-c : Corners mode\n\n");

  //fprintf("See source code for more information about setting the thresholds\n");
  //fprintf("-t <thresh> : Brightness threshold, all modes (default=20)\n");
  //fprintf("-d <thresh> : Distance threshold, smoothing mode, (default=4) (use next option instead for flat 3x3 mask)\n");
  //fprintf("-3 : Use flat 3x3 mask, edges or smoothing mode\n");
  //fprintf("-n : No post-processing on the binary edge map (runs much faster); edges mode\n");
  //fprintf("-q : Use faster (and usually stabler) corner mode; edge-like corner suppression not carried out; corners mode\n");
  //fprintf("-b : Mark corners/edges with single black points instead of black with white border; corners or edges mode\n");
  //fprintf("-p : Output initial enhancement image only; corners or edges mode (default is edges mode)\n");
...
/* }}} */
  ...
  if ( (dt>15) && (total==0) )
  {
    //fprintf("Distance_thresh (%f) too big for integer arithmetic.\n",dt);
    //fprintf("Either reduce it to <=15 or recompile with variable \"total\"\n");
    //fprintf("as a float: see top \"defines\" section.\n");
    exit(0);
  }

  if ( (2*mask_size+1>x_size) || (2*mask_size+1>y_size) )
  {
    //fprintf("Mask size (1.5*distance_thresh+1=%d) too big for image (%dx%d).\n",mask_size,x_size,y_size);
    exit(0);
  }
...
/* }}} */
  ...
  while (argindex < argc)
  {
	...
	case 'd': /* distance threshold */
          if (++argindex >= argc){
	    //fprintf ("No argument following -d\n");
	    exit(0);}
	...
	case 't': /* brightness threshold */
          if (++argindex >= argc){
	    //fprintf ("No argument following -t\n");
	    exit(0);}
