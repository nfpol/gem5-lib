//#include <linux/module.h>       /* Needed by all modules */
#include <linux/kernel.h>       /* Needed for KERN_ERR */
#include <linux/fs.h>
#include <linux/wait.h>
#include "arm_v8.h"
#include <stdio.h>
#include<stdlib.h>
#include <unistd.h>


unsigned seconds=5;

void init_pmu(void)
{
	pmu_el0_access();  // Allow access to PMU from User Mode--- works only in kernel mode
	pmu_enable_el0();  // Enable the PMU in EL0
}

void select_event(void)
{
	/*  GEM5 events are different than arm documented   ------->    look in gem5/arch/arm/ArmPMU*/

	pmu_event_config(1, 0x21);  // retired branches
	asm volatile("ISB"); 
	printf("config event 1 retired branches\n");
	pmu_event_config(2, 0x05);  // d cache refills
	asm volatile("ISB"); 
	printf("config event 2 refills in the data cache\n");
 	pmu_event_config(3, 0x08);  // retired instructions
 	asm volatile("ISB"); 
	printf("config event 3  retired instructions\n");
	pmu_event_config(4, 0x10);  // branch predictor misses
	asm volatile("ISB"); 
	printf("config event 4 branch predictor misses\n");
	pmu_event_config(5, 0x12);  // Predictable branch speculatively executed
	asm volatile("ISB"); 
	printf("config event 5 branches\n");
	pmu_event_config(6, 0x11);  // Active cycles
	asm volatile("ISB"); 
	printf("config event 6 active cycles\n");
 	pmu_event_config(7, 0x06);  // retired loads
 	asm volatile("ISB"); 
	printf("config event 7  retired loads\n");
	pmu_event_config(8, 0x13);  
	asm volatile("ISB"); 
	printf("config event 8 retired loads + stores\n");
	pmu_event_config(9, 0x21);  
	asm volatile("ISB"); 
	printf("config event 9 retired branches\n");
 	pmu_event_config(10, 0x00);  
 	asm volatile("ISB"); 
	printf("config event 10  sw increment\n");
 	pmu_event_config(11, 0x07);  // retired stores
 	asm volatile("ISB"); 
	printf("config event 11  retired stores\n");
	pmu_event_config(12, 0x02);  // i cache refills
	asm volatile("ISB"); 
	printf("config event 12 refills in the instruction cache\n");
	//sleep( seconds );
	pmu_enable_config_counter(12);
	asm volatile("ISB"); 
	pmu_enable_config_counter(1);
	asm volatile("ISB"); 
	pmu_enable_config_counter(2);
	asm volatile("ISB"); 
	pmu_enable_config_counter(3);
	asm volatile("ISB"); 
	pmu_enable_config_counter(4);
	asm volatile("ISB"); 
	pmu_enable_config_counter(5);
	asm volatile("ISB"); 
	pmu_enable_config_counter(6);
	asm volatile("ISB"); 
	pmu_enable_config_counter(7);
	asm volatile("ISB"); 
	pmu_enable_config_counter(8);
	asm volatile("ISB"); 
	pmu_enable_config_counter(9);
	asm volatile("ISB"); 
	pmu_enable_config_counter(10);
	asm volatile("ISB"); 
	pmu_enable_config_counter(11);
	asm volatile("ISB"); 
	//sleep( seconds );
}

void cycle_counter_disable(void){
	pmu_cycle_counter_disable();  // Stop CCNT cycle counter 
}

void event_counters_disable(void){
	pmu_event_counters_disable(12);   // Stop event counter 0
	asm volatile("ISB"); 
	pmu_event_counters_disable(1);   // Stop event counter 1
	asm volatile("ISB"); 
	pmu_event_counters_disable(2);   // Stop event counter 2
	asm volatile("ISB"); 
	pmu_event_counters_disable(3);   // Stop event counter 3
	asm volatile("ISB"); 
	pmu_event_counters_disable(4);   // Stop event counter 4
	asm volatile("ISB"); 
	pmu_event_counters_disable(5);   // Stop event counter 5
	asm volatile("ISB"); 
	pmu_event_counters_disable(6);   // Stop event counter 0
	asm volatile("ISB"); 
	pmu_event_counters_disable(7);   // Stop event counter 1
	asm volatile("ISB"); 
	pmu_event_counters_disable(8);   // Stop event counter 2
	asm volatile("ISB"); 
	pmu_event_counters_disable(9);   // Stop event counter 3
	asm volatile("ISB"); 
	pmu_event_counters_disable(10);   // Stop event counter 4
	asm volatile("ISB"); 
	pmu_event_counters_disable(11);   // Stop event counter 5
	asm volatile("ISB"); 
}

void reset_event_counters(void)
{	
	pmu_reset_event_counters_all();  // Reset the configurable counters
}

void reset_cycle_counter(void)
{	
	pmu_reset_cycle_counter();  // Reset the CCNT (cycle counter)
}







